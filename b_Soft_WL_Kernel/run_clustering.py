...
for i in range(len(FILE_NAMES)):
    ...
    X = np.load(os.path.join(PROJECT_ROOT, "Output", "b_Soft_WL_Kernel", "Danenberg", "Cohort_1", file_name, "X"+str(args.iteration)+".npy"))
    X_concat.append(X)
...
print(f"Concatenated feature shape {X_concat.shape}", f"{X_concat.shape[0]} subtrees from {len(np.unique(Indices))} cellular graphs" )
