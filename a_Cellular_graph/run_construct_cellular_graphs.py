...
FILE_NAMES = os.listdir(os.path.join(PROJECT_ROOT, "Input", "Single-cell", "Jackson"))
clinical = pd.read_csv(
    os.path.join(PROJECT_ROOT, "Input", "Clinical", "Jackson", "clinical.csv")
)
patient_ids = [
    patient_id
    for patient_id in list(clinical["patient_id"])
]
...
for file_name in FILE_NAMES:
    ...
    df = pd.read_csv(
        os.path.join(PROJECT_ROOT, "Input", "Single-cell", "Jackson", file_name)
    )
    Pos = df[["X", "Y"]].values
    Adj = Pos2Adj(Pos)
